...
class Xilla:
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.xilla_db = database.Database(client)
        await client.xilla_db.init()

        try:
            db = client.xilla_db
            existing = db.get("xilla.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and Xilla will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
                ...
                try:
                    if await self._check_bot(client, bot):
                        db.set("xilla.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            if os.environ.get("XILLA_NO_GIT") == "1":
                build = "unknown"
                upd = "Git disabled"
            else:
                import git
            ...
            pref = client.xilla_db.get("xilla.main", "command_prefix", None)
            ...
            try:
                ...
                await client.xilla_inline.bot.send_photo(
                    log_chat_id,
                    "https://raw.githubusercontent.com/stlity/Xilla/master/assets/xilla.png",
                    caption=(
                        "{} <b>{} started!</b>\n\n<tg-emoji emoji-id=5231065262228250587>⚙</tg-emoji> <b>GitHub commit SHA: <a"
                        ' href="https://github.com/stlity/Xilla/commit/{}">{}</a></b>\n<tg-emoji emoji-id=5873225338984599714>🔎</tg-emoji>'
                        " <b>Update status: {}</b>\n<tg-emoji emoji-id=5870903672937911120>🕶</tg-emoji> <b>Prefix:</b> <code>{}</code>"
                    ).format(
                        (
                            utils.get_platform_emoji()
                            if client.xilla_me.premium is True
                            else "🏔️ Xilla"
                        ),
                        ".".join(list(map(str, list(__version__)))),
                        build,
            ...
            logging.debug(
                ...
                client.tg_id,
                client.xilla_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
