...
class Module:
    ...
    @property
    def xilla_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def xilla_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def xilla_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def xilla_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    async def animate(
        ...
        *,
        ...
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(self.client.tg_id)  # noqa: F841
    ...
    async def _decline(
        ...
        self._db.set(
            "xilla.main",
            "declined_joins",
            list(set(self._db.get("xilla.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/xilla/declined_jr.png",
        )
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("xilla.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        await self.inline.bot.send_photo(
            self.tg_id,
            "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/xilla/join_request.png",
            caption=(
        ...
        self.xilla_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?xilla_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?xilla_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Xilla version {'{}.{}.{}'.format(*ver)}+"
                    )
                )
        ...
        if (
            ...
            and self._db.get("xilla.main", "stats", True)
            and url is not None
