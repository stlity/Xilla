...
class Quickstart(loader.Module):
    ...
    async def client_ready(self):
        self.text = lambda: self.strings["base"].format(
            ...
            if self.client.xilla_me.premium is True
            else "Xilla"
        )

        try:
            content_channel = None
            existing_channel_id = self.db.get("xilla.forums", "channel_id", None)
            ...
            if existing_channel_id:
                try:
                    ...
                    content_channel = None
                    self.db.set("xilla.forums", "forums_cache", {"xilla-userbot": {}})
            ...
            if not content_channel:
                async for dialog in self.client.iter_dialogs():
                    if dialog.title and "xilla-userbot" in dialog.title.lower():
                        ...
                        self.db.set(
                            "xilla.forums", "channel_id", int(dialog.entity.id)
                        )
                        break
            ...
            forum_entity = None
            existing_forum_id = self.db.get("xilla.forums", "forum_id", None)
            ...
            if not forum_entity:
                try:
                    ...
                    self.db.set("xilla.forums", "forum_id", int(content_channel.id))
                except Exception:
                    forum_entity = content_channel
            ...
            required_topics = [
                (
                    "Assets",
                    "🌆 Your Xilla assets will be stored here",
                    5877307202888273539,
                ),
                (
                    "Backups",
                    "💾 Your Xilla backups will be stored here",
                    5877307202888273539,
                ),
            ]
        ...
        await self.request_join(
            "xilla_talks",
            "Xilla help is only available in this chat. By agreeing to join the chat, you agree to the Xilla federation rules and if you violate them, you will be permanently banned.",
        )
    ...
    async def lang(self, call: BotInlineCall):
        if not call.data.startswith("xilla/lang/"):
            return
