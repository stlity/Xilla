...
@loader.tds
class XillaWebMod(loader.Module):
    """Inline account management"""

    strings = {"name": "XillaAccounts"}
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.xilla.api_token.ID,
            main.xilla.api_token.HASH,
            connection=main.xilla.conn,
            proxy=main.xilla.proxy,
    ...
    def _clone_switch_db(self, old_id: int, new_id: int) -> dict:
        ...
        inline_data = data.setdefault("xilla.inline", {})
    ...
    async def _save_switch_session(
        ...
        session = SQLiteSession(os.path.join(main.SESSIONS_DIR, f"xilla-{new_id}"))
        session.set_dc(
    ...
    async def _switch_account(self, client: CustomTelegramClient) -> None:
        ...
        new_id = me.id
        if old_id == new_id:
            await main.xilla.save_client_session(client, delay_restart=False)
            return

        suffix = int(time.time())
        base = Path(main.SESSIONS_DIR)
        old_session = base / f"xilla-{old_id}.session"
        old_journal = base / f"xilla-{old_id}.session-journal"
        old_config = Path(main.BASE_PATH) / f"config-{old_id}.json"
        new_session = base / f"xilla-{new_id}.session"
        new_journal = base / f"xilla-{new_id}.session-journal"
