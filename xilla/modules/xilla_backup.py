...
@loader.tds
class XillaBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "XillaBackup"}
    ...
    async def handler(self):
        try:
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "xilla/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
            ...
            logger.exception("XillaBackup failed")
            await asyncio.sleep(60)

    @loader.callback_handler()
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("xilla/backupall/restore"):
            return

        if call.data == "xilla/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "xilla/backupall/restore",
                },
            )
            return
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                with contextlib.suppress(KeyError):
                    db_data["xilla.inline"].pop("bot_token")
    ...
    def _convert(self, backup):
        fixed = re.sub(r"(hikka\.)(\S+\":)", lambda m: "xilla." + m.group(2), backup)
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["xilla.inline"].pop("bot_token")
    ...
    async def backupall(self, message: Message):
        ...
        archive.name = f"xilla-{datetime.datetime.now():%d-%m-%Y-%H-%M}.backup"
