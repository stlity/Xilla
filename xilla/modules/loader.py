...
class LoaderMod(loader.Module):
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/xilla/joined_jr.png",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?xilla_min", doc):
            ver = re.search(r"# ?scope: ?xilla_min ((?:\d+\.){2}\d+)", doc).group(1)
            ver_ = tuple(map(int, ver.split(".")))
            if main.__version__ < ver_:
                logger.error(
                    "Module %s requires Xilla %s, current version is %s",
                    module_label,
                    ver,
        ...
        module_name = f"xilla.modules.{uid}"
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "xilla_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.xilla_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.xilla_entity_cache
                    and getattr(
    ...
    def _is_core_module(self, module) -> bool:
        ...
        if not module_name.startswith("xilla.modules."):
            return False
