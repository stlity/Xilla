...
@loader.tds
class XillaInfoMod(loader.Module):
    """Show userbot info"""

    strings = {"name": "XillaInfo"}
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "banner_url",
                "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/xilla/xilla_info.png",
    ...
    async def _render_info(self, start: float) -> str:
        ...
        me = (
            '<b><a href="tg://user?id={}">{}</a></b>'.format(
                self._client.xilla_me.id,
                utils.escape_html(get_display_name(self._client.xilla_me)),
            )
            .replace("{", "")
            .replace("}", "")
        )
        ...
        return (
            ...
            if self.config["custom_message"]
                ...
                (
                    ...
                    if self._client.xilla_me.premium and self.config["show_xilla"]
                    else ""
                ),
                me=me,
                version=_version,
