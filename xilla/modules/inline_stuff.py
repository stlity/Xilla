...
class InlineStuff(loader.Module):
    ...
    @loader.command()
    async def ch_xilla_bot(self, message: Message):
        ...
        self._db.set("xilla.inline", "custom_bot", args)
        self._db.set("xilla.inline", "bot_token", None)
    ...
    async def ch_bot_token(self, message: Message):
        ...
        if not args or not re.match(r"[0-9]{8,10}:[a-zA-Z0-9_-]{34,36}", args):
            ...
            return
        self._db.set("xilla.inline", "bot_token", args)
        await utils.answer(message, self.strings["bot_updated"])
        ...
        match message.text:
            case "/start":
                await message.answer_photo(
                    "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/xilla/start_cmd.png",
                    caption=self.strings["this_is_xilla"].format(
                        (
                            "<tg-emoji emoji-id=5463379725441341739>🏔️</tg-emoji>"
                            if self._client.xilla_me.premium is True
                            else "🏔️"
                        ),
                        utils.get_platform_emoji() if self._client.xilla_me.premium is True else "Xilla",
                    ),
                    reply_markup=self.inline.generate_markup(
                        markup_obj=[
                            [
                                {
                                    "text": "GitHub",
                                    "url": "https://github.com/stlity/Xilla",
    ...
    async def reset_prefix(self, call: InlineCall, message):
        ...
        self.db.set("xilla.main", "command_prefix", ".")
