...
class Database(dict):
    ...
    async def ensure_content_channel(self):
        ...
        existing_channel_id = self.get("xilla.forums", "channel_id", None)
        ...
        if existing_channel_id:
            try:
                ...
                content_channel = None
                self.set("xilla.forums", "forums_cache", {"xilla-userbot": {}})

        if not content_channel:
            async for dialog in self._client.iter_dialogs():
                if dialog.title and "xilla-userbot" in dialog.title.lower():
                    ...
                    self.set("xilla.forums", "channel_id", int(dialog.entity.id))
                    break

        if content_channel and getattr(content_channel, "title", "").lower() == "heroku-userbot":
            try:
                await self._client(
                ...
                logger.warning("Could not rename the legacy content channel: %s", error)

        if not content_channel:
            content_channel, _ = await utils.asset_channel(
                client=self._client,
                title="xilla-userbot",
                description="🏔️ Content related to Xilla will be here",
                silent=True,
                invite_bot=True,
                avatar="https://raw.githubusercontent.com/stlity/Xilla/master/assets/xilla.png",
                forum=True,
                hide_general=True,
                _folder="xilla",
            )
            self.set("xilla.forums", "channel_id", int(content_channel.id))
    ...
    def read(self):
        ...
        try:
            ...
            if re.search(r'"(hikka\.)(\S+\":)', db):
                ...
                db = re.sub(r"(hikka\.)(\S+\":)", lambda m: "xilla." + m.group(2), db)
            if re.search(r'"(legacy\.)(\S+\":)', db):
                logging.warning("Converting db after update")
                db = re.sub(r"(legacy\.)(\S+\":)", lambda m: "xilla." + m.group(2), db)
            if re.search(r'"(heroku\.)(\S+\":)', db):
                logging.warning("Migrating Heroku database namespace to Xilla")
                db = re.sub(r"(heroku\.)(\S+\":)", lambda m: "xilla." + m.group(2), db)
    ...
    def _update_from_read(self, items: dict) -> None:
        """Update persisted data while preserving legacy project namespaces."""
        migrated_items = {}
        for key, value in items.items():
            if isinstance(key, str) and key.startswith(("hikka.", "legacy.", "heroku.")):
                key = f"xilla.{key.split('.', maxsplit=1)[1]}"
            migrated_items[key] = value

        super().update(migrated_items)
    ...
    async def store_asset(self, message: Message) -> int:
        ...
        try:
            _assets_topic_id = self.get("xilla.forums", "forums_cache", {})[
                "xilla-userbot"
            ]["Assets"]
        except (TypeError, KeyError):
            raise NoAssetsChannel("Tried to save asset to non-existing asset topic.")

        if not (_content_channel_id := self.get("xilla.forums", "channel_id", None)):
