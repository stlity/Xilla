    ...
    try:
        match name:
            ...
            case s if s.startswith("hikka"):
                return native_import("xilla" + name[5:], *args, **kwargs)
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?xilla$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.xilla_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.xilla_commands)
                inline_handlers.update(module.xilla_inline_handlers)
                callback_handlers.update(module.xilla_callback_handlers)
                watchers.extend(module.xilla_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.xilla_commands))
            )

        for _command, cmd in instance.xilla_commands.items():
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.xilla_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.xilla_watchers.values():
