...
class InlineManager(
    ...
    :type client: xilla.tl_cache.CustomTelegramClient
    :type db: xilla.database.Database
    :type allmodules: xilla.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("xilla.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.xilla_me)
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"xilla-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        try:
            ...
            self._bot_client.xilla_me = bot_me
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "Xilla":
                if any(
                    [
    ...
    async def _ping_bot(
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start xilla init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start xilla init"
                )
            except Exception:
