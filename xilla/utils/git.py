...
def _is_no_git() -> bool:
    return os.environ.get("XILLA_NO_GIT") == "1"
...
def get_git_info() -> tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/stlity/Xilla/commit/{hash_}" if hash_ else "",
    )


def get_git_hash() -> str | Literal[False]:
    """
    Get current Xilla git hash
...
def get_commit_url() -> str:
    """
    Get current Xilla git commit url
    ...
    try:
        ...
        if not hash_:
            return "Unknown"
        return f'<a href="https://github.com/stlity/Xilla/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
